...
//1800 es 3 minutos.
    ...
    public void verificar_sesion(){
        ...
        if (imei!="") {
            Servicio hilo_pedido = new Servicio();
            hilo_pedido.execute(getString(servidor) + "frmLogin.php?opcion=login", "1", imei);// parametro que recibe el doinbackground
        }else{
    ...
    public class Servicio extends AsyncTask<String,Integer,String> {
        ...
        protected void onPostExecute(String s) {
            ...
            if (s.equals("1")) {
                iniciar_Principal();
                Toast.makeText(getApplicationContext(),servidor,Toast.LENGTH_LONG).show();
                finish();//
            }
            else if(s.equals("2"))
            {
