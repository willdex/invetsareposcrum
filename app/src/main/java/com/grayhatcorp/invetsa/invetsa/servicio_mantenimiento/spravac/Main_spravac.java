...
public class Main_spravac extends AppCompatActivity implements View.OnClickListener {
    ...
    private void actualizar_lista(int dia,int mes, int anio,String filtro) {
        ...
        String consuta="select s.id, s.fecha,s.codigo,s.revision,s.id_maquina,s.id_compania,m.codigo,c.nombre from servicio_mantenimiento s,compania c,maquina m where s.id_compania=c.id and s.id_maquina=m.id and  s.formulario='SPRAVAC' and s.fecha='"+anio+"-"+mes+"-"+dia+"' ORDER BY s.id "+filtro;
        try {
